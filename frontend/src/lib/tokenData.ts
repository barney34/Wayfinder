...
export const tokenModels: TokenModel[] = [
...
export interface NiosServerGuardrail {
  model: string;
  appSize: number;
  tokens: number;
  maxDbObj: number;
  maxQPS: number;
  maxLPS: number;
  maxGridMembers: number;
  maxRPZRecords: number;
  physicalOrVirtual: string;
  key: string;
}

export const niosServerGuardrails: NiosServerGuardrail[] = [
...
export interface NiosxServerGuardrail {
  ...
  maxLPS: number;
  maxAssets: number;
  serverTokenCost: number;
  key: string;
}

export const niosxServerGuardrails: NiosxServerGuardrail[] = [
...
export interface NiosFeatureImpact {
  product: string;
  role: string;
  featureCode: string;
  featureName: string;
...
export const niosFeatureImpacts: NiosFeatureImpact[] = [
...
export function calculateNetPerformance(ratedValue: number, enabledFeatureCodes: string[], role: string): number {
...
export function getDefaultEnabledFeatures(role: string): string[] {
...
export const uddiFeatureImpacts: UddiFeatureImpact[] = [
...
export const dhcpFoAssociationLimits: Record<string, number> = {
  'TE-825':  100000,
  'TE-926':  110000,
  'TE-1415': 100000,
  'TE-1425': 100000,
...
export interface GmServiceRestriction {
  canRunServices: boolean;
  maxMembers: number;
  note: string;
}

export const gmServiceRestrictions: Record<string, GmServiceRestriction> = {
...
export interface GmServiceCheck {
  allowed: boolean;
  warning: string | null;
}

export function canGMRunServices(model: string, memberCount = 0): GmServiceCheck {
...
export interface GmObjectCalculation {
  dhcpLeaseObjects: number;
  dnsObjects: number;
  discoveryObjects: number;
  totalObjects: number;
  totalActiveIPs: number;
  totalDhcpClients: number;
  totalStaticClients: number;
}

export function calculateGMObjects(sites: Site[], dhcpPercent = 80, discoveryEnabled = false): GmObjectCalculation {
...
export interface MinimumGmModel {
  model: string;
  maxDbObj: number;
...
export function findMinimumGMModel(totalObjects: number): MinimumGmModel {
...
export interface X6ServerGuardrail {
  model: string;
  appSize: number;
  tokens: number;
  qps: number;
...
export const x6ServerGuardrails: X6ServerGuardrail[] = niosServerGuardrails.map(s => ({
  model: s.model,
  appSize: s.appSize,
  tokens: s.tokens,
  qps: s.maxQPS,
...
export const assetTiers: AssetTier[] = [
...
export function getAssetTier(knowledgeWorkers: number): AssetTier {
...
export function getAssetsPerWorker(knowledgeWorkers: number): number {
  return getAssetTier(knowledgeWorkers).assetsPerWorker;
}

// UDDI Token Calculator Data
export interface UddiManagementTokenRate {
  objectType: string;
  native: number;
  nios: number;
}

export const uddiManagementTokenRates: UddiManagementTokenRate[] = [
...
export interface UddiServerToken {
  serverType: string;
  serverSize: string;
  qps: number;
  lps: number;
...
export const uddiServerTokens: UddiServerToken[] = [
...
export const swBaseSkuMap: Record<string, string> = {
...
export const swPackageMap: Record<string, string> = {
  'DNS':               'DD',     // DNS only
  'DHCP':              'DH',     // DHCP only
...
export interface HwSkuInfo {
  hwSku: string;
  description: string;
}

export const hwSkuMap: Record<string, HwSkuInfo> = {
  ...
  // RPT — physical appliance (TE-1606 ≤10 GB, TE-2306 >10 GB)
  'TR-5005': { hwSku: 'TE-1606-HW-AC', description: 'Reporting Server Appliance' },
  // NIOS-X Virtual
...
export const unitGroupMap: Record<string, string> = {
  'GM':       'A',
  'GMC':      'A',
  'DNS':      'B',
  'DNS/DHCP': 'B',
...
export function getSwBaseSku(model: string): string {
  if (!model) return '';
...
export function getSwPackage(role: string, hasDiscovery = false, rptStorage: string | null = null): string {
  if (role === 'Reporting') {
...
export function getHwSkuInfo(model: string): HwSkuInfo {
...
export function getUnitGroup(role: string, sourceType?: string, services: string[] = []): string {

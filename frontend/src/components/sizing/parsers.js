import { defaultAssetConfigData, defaultUDDIData } from './constants';
import { getAssetTier } from '@/lib/tokenData';

export function safeParseTokens(value) {
  if (!value) return [];
  try {
...
export function safeParseTDNios(value) {
...
export function safeParseDossier(value) {
...
export function safeParseLookalike(value) {
  if (!value) return { enabled: false, quantity: 0 };
  try {
    const parsed = JSON.parse(value);
    if (typeof parsed === 'object' && parsed !== null) {
...
export function safeParseAssetConfig(value, knowledgeWorkers = 0) {
  if (!value) {
...
export function safeParseSocInsights(value) {
  const defaults = { enabled: false, collapsed: true, knowledgeWorkers: 0, overrideKW: false, assetsPerWorker: 4.0, overrideAPW: false, totalAssets: 0, calculatedTokens: 0 };
...
export function safeParseDomainTakedown(value) {
...
export function safeParseReporting(value) {
  const defaults = { enabled: false, collapsed: true, securityEventsOverride: false, securityEventsPercent: 5, securityEventsTokens: 0, reportingGrowthBufferEnabled: false, reportingGrowthBuffer: 15 };
  if (!value) return defaults;
  try {
    const parsed = JSON.parse(value);
    if (typeof parsed === 'object' && parsed !== null) {
...
export function safeParseUDDI(value) {
...
export function safeParseSites(value) {
  if (!value) return [];
  try {
    const parsed = JSON.parse(value);

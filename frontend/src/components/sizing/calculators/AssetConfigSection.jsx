...
export function AssetConfigInput({ value, onChange, questionId, knowledgeWorkers: knowledgeWorkersValue }) {
  ...
  const handleTdCloudToggle = (checked) => {
    if (!checked) onChange(JSON.stringify({ ...data, tdCloudEnabled: false, override: false, knowledgeWorkersOverride: 0, growthBufferEnabled: false, growthBuffer: 0.15, overrideAssetsPerKW: false, overrideVerifiedPerKW: false, overrideUnverifiedPerKW: false, customAssetsPerKW: 0, customVerifiedPerKW: 0, customUnverifiedPerKW: 0 }));
    ...
    <div className="space-y-3 p-3 bg-muted/30 rounded-md" data-testid={`asset-config-${questionId}`}>
      ...
      {isExpanded && data.tdCloudEnabled && (
        <div className="space-y-3 pl-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-sm font-medium">Growth Buffer</span>
            <Switch checked={data.growthBufferEnabled} onCheckedChange={(checked) => onChange(JSON.stringify({ ...data, growthBufferEnabled: checked, growthBuffer: 0.15 }))} data-testid={`switch-growth-buffer-${questionId}`} />
            {data.growthBufferEnabled && (
              <><Input type="number" step="1" min="0" max="100" value={Math.round(data.growthBuffer * 100)} onChange={(e) => onChange(JSON.stringify({ ...data, growthBuffer: Math.min(1, Math.max(0, parseFloat(e.target.value) / 100 || 0)) }))} className="w-16" /><span className="text-sm">%</span></>
            )}
          </div>
          <div className="border-t border-border pt-3 mt-3 space-y-2">
            <div className="flex items-center gap-3 pl-4"><span className="text-sm text-muted-foreground min-w-[240px]">Total Assets</span><span className="text-sm font-semibold">{totalAssets.toLocaleString()}</span><span className="text-xs text-muted-foreground">= {effectiveKW.toLocaleString()} x {estAPKW}</span></div>
            ...
            <div className="flex items-center gap-3 pl-4"><span className="text-sm text-muted-foreground min-w-[240px]">TD Cloud Tokens</span><span className="text-sm font-semibold">{(totalAssets * 3).toLocaleString()}</span><span className="text-xs text-muted-foreground">= {totalAssets.toLocaleString()} x 3</span></div>
          </div>
        </div>
      )}
    </div>

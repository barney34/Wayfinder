...
import { Eye, Download, Copy, Trash2, Check, Info, Pencil } from "lucide-react";
...
function PSARARBStatusBadge({ status, onClick, isClickable, testId }) {
  ...
  if (!isClickable || isNotRelevant) return button;

  return (
    <Tooltip>
      <TooltipTrigger asChild>{button}</TooltipTrigger>
      <TooltipContent side="top">Click to advance</TooltipContent>
    </Tooltip>
  );
}
            ...
            >
              ...
              <TableCell className="text-right">
                <div className="flex justify-end gap-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); onViewCustomer(customer.id); }} data-testid={`button-view-${customer.id}`}>
                  ...
                  {onEditCustomer && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); onEditCustomer(customer); }} data-testid={`button-edit-${customer.id}`}>
                  ...
                  {onExportCustomer && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); onExportCustomer(customer); }} data-testid={`button-export-${customer.id}`}>
                          <Download className="h-4 w-4" />
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent side="top">Export data</TooltipContent>
                    </Tooltip>
                  )}
                  {onCloneCustomer && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); onCloneCustomer({ id: customer.id, name: customer.name }); }} data-testid={`button-clone-${customer.id}`}>
                  ...
                  {onDeleteCustomer && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button size="icon" variant="ghost" className="text-destructive" onClick={(e) => { e.stopPropagation(); onDeleteCustomer({ id: customer.id, name: customer.name }); }} data-testid={`button-delete-${customer.id}`}>

...
function buildContextualOpener(driverKey, answers) {
  if (driverKey === 'optimize') {
    ...
    const dcCount         = answers['ud-5'] || 'your';

    if (hasSpreadsheets)
      return `You're tracking IP space in spreadsheets — how's that working for you day to day, especially when the environment is changing?`;
    if (hasMsIpam || hasMsDns)
      return `You're using Microsoft for DNS${hasMsDhcp ? ' and DHCP' : ''} — how is that setup holding up as your environment grows?`;
    if (hasMsDhcp)
      return `You're running Microsoft DHCP across ${dcCount} data centers — how do you handle scope management and availability across all of those today?`;
    return `How would you describe the current state of your DNS, DHCP, and IP management — what's working well and where does it get complicated?`;
  }
  ...
  if (driverKey === 'accelerate') {
    ...
    if (hasCloud) {
      ...
      return `You're running on ${cloudList} — how do you currently manage DNS and DHCP consistency between those environments and on-prem?`;
    }
    if (hasInteg) {
      const tools = integVal.split(',').map(v => v.trim()).filter(Boolean).slice(0, 2).join(' and ');
      return `You've got ${tools} in your stack — how well is your network data staying current and in sync across those tools today?`;
    }
    if (hasAutom) {
      const tools = automVal.split(',').map(v => v.trim()).filter(Boolean).join(' and ');
      return `You're using ${tools} for automation — how far have you gotten with including DNS and DHCP provisioning in those pipelines?`;
    }
    return `How would you describe where you are with cloud adoption and infrastructure automation today — what's moving well and where are things still manual?`;
  }

  if (driverKey === 'protect') {
    const ni3 = parseInt(answers['ni-3'] || '0');
    if (ni3 > 0)
      return `You have ${ni3.toLocaleString()} SNMP/SSH-managed devices — how confident are you that's the full picture of what's active on your network?`;
    return `How are you currently using DNS for security visibility, and what's your biggest concern when it comes to DNS-based threats?`;
  }

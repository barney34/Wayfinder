...
export function TopBar({ customerName, opportunity, onNameChange, onOpportunityChange, onNameBlur, onOpportunityBlur }) {
  ...
  return (
    <div className="flex-shrink-0 bg-[#1c1c1e] border-b border-[#2c2c2e]" data-testid="topbar">
      ...
      {/* Input cards row - DC/Sites larger (5fr), TS/IPs smaller (3fr) */}
      {!collapsed && (
        <div className="px-4 flex py-3 gap-3">
          {/* Spacer to match customer pill */}
          <div className="shrink-0 mr-1" style={{ width: pillWidth > 0 ? `${pillWidth}px` : '180px' }} />

          {/* Cards grid - same proportions as summary */}

...
export function AppSidebar({ 
  ...
  const isDirty = discoveryContext?.isDirty ?? false;
  const isSaving = discoveryContext?.isSaving ?? false;
  ...
  return (
    <div className={`flex flex-col h-full bg-card border-r transition-all duration-300 ${collapsed ? 'w-16' : 'w-56'}`}>
      {/* Header — logo + WAYFINDER + ThemeToggle + collapse */}
      ...
      <ScrollArea className="flex-1">
        <div className={`${collapsed ? 'px-1.5' : 'px-3'} py-3 space-y-1`}>
          ...
          {/* Customer pill — shown above Dashboard when customer is active */}
          {currentCustomer && !collapsed && (
            <button
              onClick={() => onEditRequest?.()}
              className="w-full text-left px-2 py-1.5 mb-1 rounded-lg bg-secondary border border-border hover:border-primary/40 transition-colors group"
            >
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-muted-foreground uppercase tracking-wide shrink-0 font-medium">
                  {currentNickname ? 'Nickname:' : 'Customer:'}
                </span>
                <span className="text-[11px] font-semibold text-foreground truncate">
                  {currentNickname || currentCustomer.name}
                </span>
              </div>
              {currentNickname && (
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wide shrink-0">Customer:</span>
                  <span className="text-[10px] text-muted-foreground/60 truncate">{currentCustomer.name}</span>
                </div>
              )}
              {currentOpportunity && (
                <div className="flex items-center gap-1.5">
          ...
          {collapsed ? (
            <div className="space-y-1 py-2 overflow-y-auto max-h-[180px]">
              {customers.map(c => (
                <CustomerItem
                  key={c.id}
                  customer={c}
            ...
            <div className="py-1">
              ...
              {customersOpen && (
                <div className="space-y-0.5 mt-1 max-h-[200px] overflow-y-auto">
                  ...
                  <button 
                    onClick={() => setLocation('/customers?new=1')}
      ...
      {/* Footer with Save/Export + ThemeToggle */}
      {currentCustomer && (
        <div className={`p-3 border-t ${collapsed ? 'space-y-1' : 'flex flex-col gap-2'}`}>
          <div className={collapsed ? '' : 'flex gap-2'}>
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                ...
                >
                  ...
                  {!collapsed && (
                    <span className="ml-1.5 text-xs flex items-center gap-1">
                      {saving || isSaving ? 'Saving...' : 'Save'}
                      {!saving && !isSaving && isDirty && (
